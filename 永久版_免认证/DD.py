...
class App:
    ...
    # 永久版使用本地固定授权状态，不读取激活码、硬件指纹或远程服务器。
    _act_srv_list = []
    _act_srv = ''
...
'''
    ...
    def _backup_rules_to_server(self):
        self._log('永久版已关闭原授权服务器的云端备份，请使用本地配置文件备份')
        return
        if not self._current_script:
            self._log('请先选中一个驱动')
            return
        ...
        def worker():
            try:
                server = self.settings.get('activate_server', '')
    ...
    def _restore_rules_from_server(self):
        self._log('永久版已关闭原授权服务器的云端恢复，请使用本地配置文件恢复')
        return
        if not self._current_script:
            self._log('请先选中一个驱动')
            return
    ...
    def _open_color_lib(self):
        ...
        footer_button('✕ 一键移除', delete_all, '#ef4444', 'white', 1, True)
    ...
    def _build(self):
        ...
        user_text = '永久版'
        self.activation_label = tk.Label(header, text=user_text, font=('Microsoft YaHei UI', 10, 'bold'), fg='#FFD700', bg=POP_CARD, cursor='arrow', padx=8, pady=3)
    ...
    def _online_verify_silent(key, name=''):
        return (True, '', {'ok': True, 'type': '永久版', 'expires': '永久', 'is_admin': True})
        """保留旧接口形状供兼容调用；永久版不会执行下方远程逻辑。"""
    ...
    def _show_activate_dialog():
        ...
        tk.Button(bar, text='永久版', state='disabled', bg='#E91E63', fg='white', padx=18, pady=6).pack(side='left', padx=5)
        dialog.grab_set()
        dialog.wait_window()
        return result[0]
